...
def No_normal(df):
    ...
    return  df
def test_No_normal(df):
    identifier_list = df["编号"].tolist()
    dic =['编号_null',
       '编号_一级', '编号_括号', '编号_数字+符号', '编号_数字1', '编号_罗马符号', '编号_长度2', '编号_长度3',
       '编号_长度4']
    onehot = np.zeros((len(identifier_list),len(dic)))
    for index1,value in enumerate(identifier_list):
        value = '编号'+"_"+value
        if value in dic:
            ...
            onehot[index1][:] = 100
    No_df = pd.DataFrame(onehot,columns=dic)
    df = pd.concat([df,No_df], axis=1)
    return df
def datanormal(datapath,resultpath,step):
    ...
    dic ={'中文摘要标题': 1, '中文关键词': 2, '英文摘要标题': 3, '英文关键词': 4, '一级标题': 5, '二级标题': 6, '文本段': 7, '三级标题': 8, '图片': 9, '图题': 10, '表题': 11, '表格': 12, '四级标题': 13, '程序代码': 14, '五级标题': 15, '公式': 16, '论文名称': 17, '姓名': 18, '单位': 19, '邮箱': 20
          ,'中文摘要内容':21,'英文摘要内容':22}
